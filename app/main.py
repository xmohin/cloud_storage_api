...
from app.services.file_service import run_trash_purge_loop
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        trash_cleanup_task = asyncio.create_task(run_trash_purge_loop(db.get_database()))
        ...
        if not email_client.is_configured:
            logger.warning(
                "STARTUP WARNING: Brevo email is NOT configured — "
                "registration OTPs, password-reset OTPs, and share-link "
                "emails will silently fail to send. Set BREVO_API_KEY to fix."
            )
    ...
    services_to_stop = []
    services_to_stop.append(("Trash Cleanup Loop", _cancel_task(trash_cleanup_task)))
