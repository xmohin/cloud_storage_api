...
async def list_by_type(file_type: FileType, user: CurrentUserDep, db: DatabaseDep, pagination: PaginationDep):
    res = await file_service.list_by_type(db, user["_id"], file_type, pagination.skip, pagination.per_page)
    files = [FileMetadata(**f) for f in res["files"]]
    return ApiResponse(data={"files": files, "total": res["total"]})
