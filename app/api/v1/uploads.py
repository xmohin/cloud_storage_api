...
@router.post("/small")
async def upload_small_file(user: CurrentUserDep, db: DatabaseDep, file: UploadFile = File(...)):
    result = await upload_service.handle_small_upload(db, file, user["_id"])
...
async def init_chunked_upload(payload: UploadInitRequest, user: CurrentUserDep, db: DatabaseDep):
    result = await upload_service.init_chunked_upload(
        db, payload.filename, payload.file_size, payload.mime_type, payload.total_chunks, user["_id"]
    )
