...
from app.models.schemas import ApiResponse, OTPRequest, VerifyOTP, PasswordResetRequest, PasswordResetConfirm, RefreshTokenRequest, UserProfile, TokenResponse, UserLogin, UserRegister
...
async def refresh_token(payload: RefreshTokenRequest, db: DatabaseDep):
    ...
    await db.blacklist.update_one({"jti": token_data["jti"]}, {"$set": {"jti": token_data["jti"], "expires_at": datetime.fromtimestamp(token_data["exp"], tz=timezone.utc)}}, upsert=True)
...
async def logout(payload: RefreshTokenRequest, user: CurrentUserDep, db: DatabaseDep):
    if user.get("_jti"): await db.blacklist.update_one({"jti": user["_jti"]}, {"$set": {"jti": user["_jti"], "expires_at": datetime.now(timezone.utc) + timedelta(minutes=settings.JWT_ACCESS_TOKEN_EXPIRE_MINUTES)}}, upsert=True)
    try:
        refresh_data = security.decode_token(payload.refresh_token, "refresh")
        await db.blacklist.update_one({"jti": refresh_data["jti"]}, {"$set": {"jti": refresh_data["jti"], "expires_at": datetime.fromtimestamp(refresh_data["exp"], tz=timezone.utc)}}, upsert=True)
        await db.sessions.update_one({"refresh_jti": refresh_data["jti"]}, {"$set": {"is_active": False}})
    except Exception: pass
    return ApiResponse(message="Logged out successfully")

@router.post("/send-otp")
async def send_otp(payload: OTPRequest, db: DatabaseDep, email_service: EmailServiceDep):
    user = await db.users.find_one({"email": payload.email})
...
async def verify_otp(payload: VerifyOTP, db: DatabaseDep):
    user = await db.users.find_one({"email": payload.email})
