...
async def search(q: str, user: CurrentUserDep, db: DatabaseDep, pagination: PaginationDep):
    ...
    return ApiResponse(data=FileListResponse(files=[FileMetadata(**f) for f in res["files"]], total=res["total"], page=pagination.page, limit=pagination.per_page, per_page=pagination.per_page, has_next=(pagination.skip + pagination.per_page) < res["total"]))
...
async def search_images(q: str, user: CurrentUserDep, db: DatabaseDep, pagination: PaginationDep):
    ...
    return ApiResponse(data=FileListResponse(files=[FileMetadata(**f) for f in files], total=total, page=pagination.page, limit=pagination.per_page, per_page=pagination.per_page, has_next=(pagination.skip + pagination.per_page) < total))
