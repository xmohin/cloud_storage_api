...
async def delete_user(user_id: str, admin: AdminUserDep, db: DatabaseDep):
    """Deactivate user and cascade: sessions, shares, soft-delete files (same as self-service account delete)."""
    ...
    result = await db.users.update_one(
        {"_id": user_id},
        {"$set": {"is_active": False, "deleted_at": now, "storage_used_bytes": 0}},
    )
    if result.matched_count == 0:
        raise HTTPException(status.HTTP_404_NOT_FOUND, detail="User not found")
    await db.sessions.update_many({"user_id": user_id}, {"$set": {"is_active": False}})
    await db.shares.update_many(
        {"owner_id": user_id, "is_active": True}, {"$set": {"is_active": False}}
    )
