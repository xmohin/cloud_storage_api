...
class PaginationParams:
    def __init__(
        ...
        self.page = page
        self.limit = limit
        self.per_page = limit  # alias used by files.py / folders.py / search.py
...
async def get_current_user(
    ...
    jti = payload.get("jti")
    if jti and await database.blacklist.find_one({"jti": jti}):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
    ...
    # Stash the access token's jti so logout()/delete_all_sessions() can
    # blacklist *this* token — previously always absent, so that block in
    # auth.logout() was permanently dead and revoked tokens stayed valid
