...
class Database:
    ...
    async def connect(cls) -> None:
        if cls._client is not None: return
        ...
        cls._client = AsyncIOMotorClient(settings.MONGODB_URI, maxPoolSize=settings.MONGODB_MAX_POOL_SIZE, minPoolSize=settings.MONGODB_MIN_POOL_SIZE, maxIdleTimeMS=settings.MONGODB_MAX_IDLE_TIME_MS, serverSelectionTimeoutMS=settings.MONGODB_SERVER_SELECTION_TIMEOUT_MS, retryWrites=True, retryReads=True, appName=settings.APP_NAME)
        try:
    ...
    async def _ensure_indexes(cls) -> None:
        ...
        await db.files.create_index([("owner_id", 1), ("parent_id", 1), ("deleted_at", 1), ("is_folder", -1), ("original_name", 1)], name="idx_directory_list")
    ...
    def get_database(cls) -> AsyncIOMotorDatabase:
        if cls._database is None: raise RuntimeError("Database not initialised")
...
class BrevoEmailClient:
    ...
    async def connect(self) -> None:
        if self._http is not None: return
        if not self._api_key:
            logger.warning("Brevo API key not configured")
            return
        self._http = httpx.AsyncClient(base_url=self._base_url, headers={"api-key": self._api_key, "Content-Type": "application/json", "Accept": "application/json"}, timeout=httpx.Timeout(connect=5.0, read=30.0, write=10.0, pool=5.0), limits=httpx.Limits(max_connections=20, max_keepalive_connections=5))
    ...
    async def send_email(self, to_email: str, subject: str, html_content: str, to_name: str | None = None, text_content: str | None = None) -> bool:
        if self._http is None: return False
        payload: dict[str, Any] = {"sender": {"email": self._sender_email, "name": self._sender_name}, "to": [{"email": to_email, **({"name": to_name} if to_name else {})}], "subject": subject, "htmlContent": html_content}
        if text_content: payload["textContent"] = text_content
        try:
