...
class OTPRequest(BaseModel):
    email: EmailStr; purpose: str = "verification"
class VerifyOTP(BaseModel):
    email: EmailStr; otp: str = Field(..., min_length=6, max_length=6)
class UserProfile(ORMModel):
    id: str = Field(..., alias="_id"); username: str; email: EmailStr; role: UserRole = UserRole.USER
    is_verified: bool = False; is_active: bool = True; storage_used_bytes: int = 0
    storage_quota_bytes: int = Field(default=5 * 1024 * 1024 * 1024)
    avatar_message_id: int | None = None; created_at: datetime; updated_at: datetime
...
class FileMetadata(ORMModel):
    id: str = Field(..., alias="_id"); owner_id: str; parent_id: str | None = None; original_name: str
    file_type: FileType; mime_type: str | None = None; size_bytes: int = 0; file_hash: str | None = None
    is_folder: bool = False; is_favorite: bool = False; status: FileStatus = FileStatus.COMPLETED
    telegram_message_id: int | None = None; thumbnail_message_id: int | None = None
...
class Notification(ORMModel):
    id: str = Field(..., alias="_id"); user_id: str; message: str; is_read: bool = False; created_at: datetime

# ── Admin ──
class AdminUserUpdate(BaseModel):
    is_active: bool | None = None; role: UserRole | None = None; storage_quota_bytes: int | None = None
...
class PinChange(BaseModel):
    current_pin: str; new_pin: str = Field(..., min_length=4, max_length=6, pattern=r"^\d+$")

# ── Generic ──
class ApiResponse(BaseModel, Generic[T]):
    success: bool = True; message: str | None = None; data: T | None = None
class HealthResponse(BaseModel):
    status: str = "healthy"; version: str = "1.0.0"; services: dict[str, str] = Field(default_factory=dict)
