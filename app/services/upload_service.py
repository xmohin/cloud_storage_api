...
class UploadService:
    def __init__(self): self._cleanup_task: Optional[asyncio.Task] = None
    ...
    def _get_upload_dir(self, upload_id: str) -> str:
        path = os.path.join(TEMP_UPLOAD_DIR, upload_id); os.makedirs(path, exist_ok=True); return path

    async def handle_small_upload(self, file: UploadFile, user_id: str) -> dict:
        upload_id = str(uuid.uuid4()); upload_dir = self._get_upload_dir(upload_id)
        temp_path = os.path.join(upload_dir, file.filename); hasher = hashlib.sha256(); size_bytes = 0
        async with aiofiles.open(temp_path, 'wb') as out_file:
            while chunk := await file.read(CHUNK_SIZE):
                await out_file.write(chunk); hasher.update(chunk); size_bytes += len(chunk)
        ...
        if existing_file:
            ...
            shutil.rmtree(upload_dir); return {"file_id": new_file_doc["_id"], "is_duplicate": True}
    ...
    async def init_chunked_upload(self, filename: str, file_size: int, mime_type: str, total_chunks: int, user_id: str) -> dict:
        ...
        self._get_upload_dir(upload_id); return {"upload_id": upload_id, "chunk_size": CHUNK_SIZE}
    ...
    async def upload_chunk(self, upload_id: str, chunk_index: int, file: UploadFile, user_id: str) -> dict:
        ...
        if not upload: raise HTTPException(status.HTTP_404_NOT_FOUND, detail="Upload session not found")
        if upload["status"] == "paused": raise HTTPException(status.HTTP_400_BAD_REQUEST, detail="Upload is paused.")
        chunk_path = os.path.join(TEMP_UPLOAD_DIR, upload_id, f"chunk_{chunk_index}")
        async with aiofiles.open(chunk_path, 'wb') as out_file:
            while chunk := await file.read(CHUNK_SIZE): await out_file.write(chunk)
    ...
    async def resume_upload(self, upload_id: str, user_id: str) -> dict:
        upload = await db.uploads.find_one({"_id": upload_id, "user_id": user_id})
        if not upload or upload["status"] != "paused": raise HTTPException(status.HTTP_400_BAD_REQUEST, detail="Upload not found or not paused")
    ...
    async def complete_chunked_upload(self, upload_id: str, user_id: str) -> dict:
        upload = await db.uploads.find_one({"_id": upload_id, "user_id": user_id})
        if not upload or len(upload["received_chunks"]) != upload["total_chunks"]: raise HTTPException(status.HTTP_400_BAD_REQUEST, detail="Not all chunks received")
    ...
    async def _process_chunked_upload(self, upload_id: str, user_id: str, upload: dict):
        try:
            upload_dir = os.path.join(TEMP_UPLOAD_DIR, upload_id); final_path = os.path.join(upload_dir, upload["filename"]); hasher = hashlib.sha256()
            async with aiofiles.open(final_path, 'wb') as out_file:
                for i in range(upload["total_chunks"]):
                    ...
                    async with aiofiles.open(chunk_path, 'rb') as in_file:
                        while chunk := await in_file.read(CHUNK_SIZE): await out_file.write(chunk); hasher.update(chunk)
            ...
            if existing_file:
                ...
                await db.uploads.update_one({"_id": upload_id}, {"$set": {"status": "completed", "file_id": new_file_doc["_id"], "is_duplicate": True}}); return
    ...
    async def _create_file_record(self, user_id: str, filename: str, mime_type: str, size_bytes: int, file_hash: str, tg_msg_id: int | None, tg_thumb_id: int | None) -> dict:
        file_doc = {"_id": str(uuid.uuid4()), "owner_id": user_id, "original_name": filename, "filename": f"{uuid.uuid4().hex}_{filename}", "file_type": self._get_file_type(mime_type), "mime_type": mime_type, "size_bytes": size_bytes, "file_hash": file_hash, "telegram_message_id": tg_msg_id, "thumbnail_message_id": tg_thumb_id, "status": "completed" if tg_msg_id else "uploading", "created_at": datetime.now(timezone.utc), "updated_at": datetime.now(timezone.utc)}
    ...
    async def get_upload_status(self, upload_id: str, user_id: str) -> dict:
        ...
        return {"upload_id": upload_id, "status": upload["status"], "received_chunks": upload["received_chunks"], "total_chunks": upload.get("total_chunks", 0), "progress": progress, "file_id": upload.get("file_id"), "telegram_task_id": upload.get("telegram_task_id")}
    ...
    async def _stale_upload_cleanup(self):
        while True:
            ...
            except Exception as e: logger.error(f"Stale cleanup error: {e}")
