...
# Mirrors the db.uploads TTL window (see database.py) — a per-upload
# directory older than this with no recent activity is almost certainly
# left over from a crashed worker or an abandoned chunked upload.
STALE_UPLOAD_DIR_MAX_AGE_SECONDS = 86400
UPLOAD_DIR_CLEANUP_INTERVAL_SECONDS = 3600


class UploadService:
    def __init__(self):
        self._cleanup_task: Optional[asyncio.Task] = None
        # The event loop only holds a *weak* reference to tasks — anything
        # created with asyncio.create_task() and not kept anywhere else can
        # be garbage-collected mid-run. This set gives fire-and-forget tasks
        # (e.g. complete_chunked_upload's finalize step) a strong reference
        # for their lifetime; each task removes itself once it's done.
    ...
    async def start(self):
        ...
        if self._cleanup_task is None:
            # This task was declared in __init__ but nothing ever created
            # it, so stale per-upload directories accumulated on disk
            # forever (issue #19).
    ...
    async def _cleanup_loop(self):
        while True:
            try:
                await self._cleanup_stale_dirs()
            except asyncio.CancelledError:
                break
            except Exception as e:
                logger.error("upload_dir_cleanup_error", error=str(e))
            await asyncio.sleep(UPLOAD_DIR_CLEANUP_INTERVAL_SECONDS)

    async def _cleanup_stale_dirs(self) -> int:
        """Removes per-upload temp directories that haven't been touched in
        STALE_UPLOAD_DIR_MAX_AGE_SECONDS. An actively-uploading directory's
        mtime keeps advancing as chunks land, so this only catches ones that
        are genuinely abandoned (crashed worker, client gave up mid-upload,
        or the pre-fix leaks described in issue #19)."""
        if not os.path.isdir(TEMP_UPLOAD_DIR):
            return 0
        now = time.time()
        removed = 0
        for entry in os.listdir(TEMP_UPLOAD_DIR):
            dir_path = os.path.join(TEMP_UPLOAD_DIR, entry)
            ...
            try:
                ...
                continue
            if age < STALE_UPLOAD_DIR_MAX_AGE_SECONDS:
                continue
            shutil.rmtree(dir_path, ignore_errors=True)
            removed += 1
        if removed:
            logger.info("Cleaned up stale upload directories", count=removed)
        return removed
    ...
    async def _get_quota_status(self, db: AsyncIOMotorDatabase, user_id: str) -> tuple[int, int]:
        user = await db.users.find_one({"_id": user_id}, {"storage_used_bytes": 1, "storage_quota_bytes": 1})
        if not user:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")
        return user.get("storage_used_bytes", 0), user.get("storage_quota_bytes", 0)
    ...
    async def handle_small_upload(
        ...
        used_bytes, quota_bytes = await self._get_quota_status(db, user_id)
    ...
    async def init_chunked_upload(
        ...
        await db.uploads.insert_one({
            ...
            "status": "uploading",
            "created_at": now,
            # Refreshed on every chunk/pause/resume below — the TTL index in
            # database.py expires on this field rather than the static
            # created_at, so a slow-but-active upload no longer gets its
            # session doc deleted out from under it mid-flight (issue #19).
            "last_activity_at": now,
        })
    ...
    async def upload_chunk(
        ...
        if chunk_index < 0 or chunk_index >= upload.get("total_chunks", 0):
        ...
        if oversized:
            ...
            raise HTTPException(status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE, detail="Chunk exceeds maximum chunk size")

        await db.uploads.update_one(
            {"_id": upload_id},
            {"$addToSet": {"received_chunks": chunk_index}, "$set": {"last_activity_at": datetime.now(timezone.utc)}},
        )
    ...
    async def pause_upload(self, db: AsyncIOMotorDatabase, upload_id: str, user_id: str) -> dict:
        await db.uploads.update_one(
            {"_id": upload_id, "user_id": user_id, "status": "uploading"},
            {"$set": {"status": "paused", "last_activity_at": datetime.now(timezone.utc)}},
        )
    ...
    async def resume_upload(self, db: AsyncIOMotorDatabase, upload_id: str, user_id: str) -> dict:
        ...
        await db.uploads.update_one(
            {"_id": upload_id},
            {"$set": {"status": "uploading", "last_activity_at": datetime.now(timezone.utc)}},
        )
    ...
    async def complete_chunked_upload(self, db: AsyncIOMotorDatabase, upload_id: str, user_id: str) -> dict:
        ...
        if not upload:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Upload session not found")
        if len(upload.get("received_chunks", [])) != upload.get("total_chunks"):
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Not all chunks received")

        # Atomic claim: only one concurrent complete() may transition out of
        # uploading/paused into processing. Without this, two parallel complete
        # requests both spawned _process_chunked_upload → double file records
        # and double quota charge.
        claimed = await db.uploads.find_one_and_update(
            {
                "_id": upload_id,
                "user_id": user_id,
        ...
        if claimed is None:
            current = await db.uploads.find_one({"_id": upload_id, "user_id": user_id}, {"status": 1})
            status_now = (current or {}).get("status")
            if status_now in ("processing", "completed"):
                raise HTTPException(
                    status_code=status.HTTP_409_CONFLICT,
                    detail="Upload is already being completed",
                )
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Upload cannot be completed from status '{status_now}'",
            )

        self._spawn_background_task(
            self._process_chunked_upload(db, upload_id, user_id, claimed)
        )
    ...
    async def _process_chunked_upload(self, db: AsyncIOMotorDatabase, upload_id: str, user_id: str, upload: dict):
        try:
            ...
            if existing_file:
                new_file_doc = await self._create_file_record(
                    db, user_id, upload["filename"], upload["mime_type"], actual_size,
                ...
                await db.uploads.update_one(
                    {"_id": upload_id},
                    {"$set": {
                        "status": "completed", "file_id": new_file_doc["_id"], "is_duplicate": True,
                        "last_activity_at": datetime.now(timezone.utc),
                    }},
                )
                return
            ...
            file_doc = await self._create_file_record(
                db, user_id, upload["filename"], upload["mime_type"], actual_size, file_hash, None, None, parent_id
            )
            ...
            await db.uploads.update_one(
                {"_id": upload_id},
                {"$set": {
                    "status": "processing", "file_id": file_doc["_id"], "telegram_task_id": task_id,
                    "last_activity_at": datetime.now(timezone.utc),
                }},
            )
        except Exception as e:
            await db.uploads.update_one(
                {"_id": upload_id},
                {"$set": {"status": "failed", "error": str(e), "last_activity_at": datetime.now(timezone.utc)}},
            )
    ...
    async def _create_file_record(
        ...
        file_doc = {
            ...
            "file_hash": file_hash,
            # Must be written explicitly, not left to the schema's default.
            # file_service's list/recent/type/statistics queries all filter
            # with {"is_folder": False} (or True) directly against Mongo —
            # a document that never had the field at all does not match an
            # equality query for either value, so an uploaded file with no
            # is_folder key was invisible to every one of those endpoints.
            "is_folder": False,
            "is_favorite": False,
        ...
        # Atomic quota check-and-charge: concurrent uploads can no longer each
        # pass a soft pre-check and then all $inc past the quota. $expr compares
        # the live storage_used_bytes against quota in the same write that
        # increments it; a lost race returns None and we refuse the record.
        if size_bytes > 0:
            charged = await db.users.find_one_and_update(
                {
                    ...
                    "$expr": {
                        "$lte": [
                            {"$add": [{"$ifNull": ["$storage_used_bytes", 0]}, size_bytes]},
                            {"$ifNull": ["$storage_quota_bytes", 0]},
                        ]
                    },
                },
                {"$inc": {"storage_used_bytes": size_bytes}},
            )
            if charged is None:
                raise HTTPException(
                    status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE,
        ...
        try:
            ...
            # Roll back the quota charge if the file doc couldn't be written.
            if size_bytes > 0:
                await db.users.update_one(
                    {"_id": user_id}, {"$inc": {"storage_used_bytes": -size_bytes}}
                )
            raise
        return file_doc
    ...
    async def get_upload_status(self, db: AsyncIOMotorDatabase, upload_id: str, user_id: str) -> dict:
        ...
        progress = 1.0 if upload.get("status") == "completed" else float(upload.get("telegram_progress") or 0.0)
        if upload.get("status") == "processing" and upload.get("telegram_task_id"):
            # Cross-worker safe: memory on this process, else Mongo tasks collection
            task = await telegram_service.get_task_status_async(upload["telegram_task_id"])
            if task:
