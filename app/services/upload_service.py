...
class UploadService:
    ...
    async def handle_small_upload(self, db: AsyncIOMotorDatabase, file: UploadFile, user_id: str) -> dict:
        ...
        if existing_file:
            new_file_doc = await self._create_file_record(
                db, user_id, file.filename, file.content_type or "application/octet-stream",
        ...
        file_doc = await self._create_file_record(
            db, user_id, file.filename, file.content_type or "application/octet-stream",
            size_bytes, file_hash, None, None
        )
    ...
    async def _process_chunked_upload(self, db: AsyncIOMotorDatabase, upload_id: str, user_id: str, upload: dict):
        try:
            ...
            file_doc = await self._create_file_record(
                db, user_id, upload["filename"], upload["mime_type"], upload["file_size"], file_hash, None, None
            )
