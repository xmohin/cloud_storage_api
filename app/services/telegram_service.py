...
class TelegramService:
    ...
    def _cleanup_upload_path(file_path: str) -> None:
        ...
        Every temp file this worker uploads lives in its own per-upload
        directory (see UploadService._get_upload_dir) holding nothing else,
        so removing that directory removes the file too — unlike a bare
        os.remove(file_path), it doesn't leave the now-empty directory
        behind. Restricted to TEMP_STORAGE_PATH as a safety check before any
        recursive delete (issue #19).
        """
        if not file_path:
            return
    ...
    def _is_image_path(file_path: str) -> bool:
        """Cheap extension-based check so we don't even attempt thumbnailing non-images."""
        mime_type, _ = mimetypes.guess_type(file_path)
        return bool(mime_type and mime_type.startswith("image"))

    def _extract_thumbnail(self, file_path: str) -> Optional[bytes]:
        """Synchronous, CPU/disk-bound. Callers MUST run this via asyncio.to_thread
        ...
        while self._is_running:
            ...
            try:
                ...
                if self._is_image_path(task.file_path):
                    # PIL is synchronous/blocking — run off the event loop thread.
                ...
                if task.file_id:
                    ...
                    await db.uploads.update_many(
                        {"file_id": task.file_id}, {"$set": {"status": "completed"}}
                    )
            except Exception as e:
                ...
                if task.file_id:
                    await db.files.update_one(
                        {"_id": task.file_id}, {"$set": {"status": "failed"}}
                    )
            finally:
                # Runs on both success and failure — previously the file was
                # only removed (os.remove, not even the directory) on the
                # success path, so a failed Telegram send leaked the temp
                # file forever, and even success left the now-empty
                # per-upload directory behind (issue #19).
