...
class FileService:
    ...
    async def rename_file(db: AsyncIOMotorDatabase, file_id: str, user_id: str, new_name: str) -> dict:
        ...
        await db.files.update_one(
            {"_id": file_id},
            {"$set": {"original_name": safe_name, "updated_at": datetime.now(timezone.utc)}},
        )
        file_doc["original_name"] = safe_name
        return file_doc
    ...
    async def copy_file(db: AsyncIOMotorDatabase, file_id: str, user_id: str, new_parent_id: Optional[str]) -> dict:
        ...
        # Incomplete / failed uploads must not be copied — they may lack a
        # telegram_message_id and can carry quota_refunded=True, which would
        # leave the copy charged in storage_used but flagged as already refunded.
        if not file_doc.get("is_folder"):
            if file_doc.get("status") != "completed" or not file_doc.get("telegram_message_id"):
                raise HTTPException(
                    status.HTTP_409_CONFLICT,
                    detail="Only completed files can be copied",
                )
        ...
        if total_bytes:
            # Atomic check-and-charge (same pattern as upload_service) so two
            # concurrent copies cannot both pass a soft pre-check and blow past quota.
            charged = await db.users.find_one_and_update(
                {
                    "_id": user_id,
                    "$expr": {
                        "$lte": [
                            {"$add": [{"$ifNull": ["$storage_used_bytes", 0]}, total_bytes]},
                            {"$ifNull": ["$storage_quota_bytes", 0]},
                        ]
                    },
                },
                {"$inc": {"storage_used_bytes": total_bytes}},
            )
            if charged is None:
        ...
        try:
            ...
            if total_bytes:
                await db.users.update_one(
                    {"_id": user_id}, {"$inc": {"storage_used_bytes": -total_bytes}}
                )
            raise
    ...
    async def _subtree_size(db: AsyncIOMotorDatabase, file_doc: dict, user_id: str) -> int:
        """Total size in bytes of file_doc, or of every copyable non-folder
        descendant if file_doc is a folder (folders themselves are always
        stored with size_bytes=0). Failed / incomplete files are skipped so
        quota is only charged for records that will actually be duplicated.
        """
        if not file_doc.get("is_folder"):
            if file_doc.get("status") != "completed" or file_doc.get("quota_refunded"):
                return 0
    ...
    async def _copy_subtree(
        ...
        # Skip incomplete / already-refunded leaves when walking a folder tree.
        if not file_doc.get("is_folder"):
            if file_doc.get("status") != "completed" or not file_doc.get("telegram_message_id"):
                return None
        ...
        # Never inherit refund/failure bookkeeping — this is a fresh charged copy.
        new_doc.pop("quota_refunded", None)
    ...
    async def create_folder(db: AsyncIOMotorDatabase, user_id: str, name: str, parent_id: Optional[str]) -> dict:
        ...
        if parent_id:
            ...
            if not parent_doc.get("is_folder"):
                raise HTTPException(status.HTTP_400_BAD_REQUEST, detail="Parent must be a folder")
        folder_doc = {
            "_id": str(uuid.uuid4()),
            "owner_id": user_id,
            "parent_id": parent_id,
    ...
    @staticmethod
    def _quota_still_charged(file_doc: dict) -> bool:
        """True if this file's size is still counted in storage_used_bytes.

        Telegram upload failure refunds quota and sets quota_refunded=True so
        a later trash/permanent-delete must not subtract size_bytes again.
        """
        if file_doc.get("is_folder"):
            return False
        if file_doc.get("quota_refunded"):
            return False
        return True
    ...
    async def move_to_trash(db: AsyncIOMotorDatabase, file_id: str, user_id: str) -> dict:
        file_doc = await FileService._get_file_doc(db, file_id, user_id)
        if file_doc.get("is_folder"):
            await FileService._recursive_trash(db, file_id, user_id)
        ...
        await db.files.update_one(
            {"_id": file_id},
            {
                "$set": {
                    ...
                    "deleted_expires_at": deleted_at + timedelta(days=TRASH_EXPIRY_DAYS),
                    "updated_at": deleted_at,
                }
            },
        )
        if FileService._quota_still_charged(file_doc):
            await db.users.update_one(
                {"_id": user_id},
                {"$inc": {"storage_used_bytes": -file_doc.get("size_bytes", 0)}},
            )
    ...
    async def _recursive_trash(db: AsyncIOMotorDatabase, folder_id: str, user_id: str):
        children = await db.files.find(
            {"owner_id": user_id, "parent_id": folder_id, "deleted_at": None}
        ).to_list(None)
        for child in children:
            if child.get("is_folder"):
                await FileService._recursive_trash(db, child["_id"], user_id)
            elif FileService._quota_still_charged(child):
                await db.users.update_one(
                    {"_id": user_id},
                    {"$inc": {"storage_used_bytes": -child.get("size_bytes", 0)}},
                )
            deleted_at = datetime.now(timezone.utc)
            await db.files.update_one(
                {"_id": child["_id"]},
    ...
    async def restore_from_trash(db: AsyncIOMotorDatabase, file_id: str, user_id: str) -> dict:
        ...
        # CRITICAL #5: restoring a trashed ancestor (above) already sweeps
        # file_id back up via that call's own _recursive_restore, since
        # file_id is still deleted_at != None at that point and matches its
        # query. Re-check file_id's *current* state before touching it again
        # — proceeding unconditionally here double-$inc'd storage_used_bytes
        # (and, for a folder, redundantly re-walked an already-restored
        # subtree) any time the restored item had a trashed ancestor.
        current = await db.files.find_one({"_id": file_id, "owner_id": user_id})
        if current and current.get("deleted_at") is None:
            file_doc["deleted_at"] = None
            return file_doc
        await db.files.update_one(
            {"_id": file_id},
            {"$set": {"deleted_at": None, "deleted_expires_at": None, "updated_at": datetime.now(timezone.utc)}},
        )
        if file_doc.get("is_folder"):
            await FileService._recursive_restore(db, file_id, user_id)
        elif FileService._quota_still_charged(file_doc):
            # Skip re-charge for failed uploads that already refunded quota
            await db.users.update_one(
                {"_id": user_id},
                {"$inc": {"storage_used_bytes": file_doc.get("size_bytes", 0)}},
            )
    ...
    async def _recursive_restore(db: AsyncIOMotorDatabase, folder_id: str, user_id: str):
        children = await db.files.find(
            {"owner_id": user_id, "parent_id": folder_id, "deleted_at": {"$ne": None}}
        ).to_list(None)
        for child in children:
            if child.get("is_folder"):
                await FileService._recursive_restore(db, child["_id"], user_id)
            elif FileService._quota_still_charged(child):
                await db.users.update_one(
                    {"_id": user_id},
                    {"$inc": {"storage_used_bytes": child.get("size_bytes", 0)}},
            ...
            await db.files.update_one(
                {"_id": child["_id"]},
                {"$set": {"deleted_at": None, "deleted_expires_at": None}},
            )
    ...
    async def permanent_delete(db: AsyncIOMotorDatabase, file_id: str, user_id: str):
        file_doc = await FileService._get_file_doc(db, file_id, user_id, include_deleted=True)
        if file_doc.get("is_folder"):
            await FileService._recursive_permanent_delete(db, file_id, user_id)
        else:
            # Only debit if still charged: not already trashed, and not a
            # failed-upload that already refunded via quota_refunded.
            if not file_doc.get("deleted_at") and FileService._quota_still_charged(file_doc):
                await db.users.update_one(
                    {"_id": user_id},
                    {"$inc": {"storage_used_bytes": -file_doc.get("size_bytes", 0)}},
                )
            if file_doc.get("telegram_message_id") and telegram_service.client:
                duplicates = await db.files.count_documents(
                    {"telegram_message_id": file_doc["telegram_message_id"], "_id": {"$ne": file_id}}
                )
                if duplicates == 0:
                    try:
                        await telegram_service.client.delete_messages(
                            entity=settings.TELEGRAM_STORAGE_CHANNEL_ID,
                            message_ids=[file_doc["telegram_message_id"]],
                        )
                    except Exception as e:
                        logger.error(f"Failed to delete TG msg: {e}")
    ...
    async def _recursive_permanent_delete(db: AsyncIOMotorDatabase, folder_id: str, user_id: str):
        ...
        for child in children:
            if child.get("is_folder"):
                await FileService._recursive_permanent_delete(db, child["_id"], user_id)
            else:
                if not child.get("deleted_at") and FileService._quota_still_charged(child):
                    await db.users.update_one(
                        {"_id": user_id},
                        {"$inc": {"storage_used_bytes": -child.get("size_bytes", 0)}},
                    )
                if child.get("telegram_message_id") and telegram_service.client:
                    duplicates = await db.files.count_documents(
                        {"telegram_message_id": child["telegram_message_id"], "_id": {"$ne": child["_id"]}}
                    )
                    if duplicates == 0:
                        try:
                            await telegram_service.client.delete_messages(
                                entity=settings.TELEGRAM_STORAGE_CHANNEL_ID,
                                message_ids=[child["telegram_message_id"]],
                            )
                        except Exception:
                            pass
